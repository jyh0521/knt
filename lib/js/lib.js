...
function paging(param, callback) {
	...
	$("#" + divId + " a#" + currentPage).css({ "background-color": "rgb(95 95 95)", "color":"rgb(251 251 251)", "border": "solid 1px" });    // 현재 페이지 표시
...
/*
	...
	일때,
	id=abcd&pw=1234 형식으로 자동으로 parameter를 만들어 반환
*/
function makeParam(param) {
	let result = '';

	result += Object.keys(param)[0] + '=' + param[Object.keys(param)[0]];

	for(let i = 1; i < Object.keys(param).length; i++) {
		result += '&' + Object.keys(param)[i] + '=' + param[Object.keys(param)[i]];
	}
...
/*
	뒤로가기를 위해 pushState를 하는 함수
	page에 고유한 페이지의 이름을 넣으면 해당 페이지의 뒤로가기 기능이 구현된다.
*/
function makeBackEvent(page) {
	if(nowPage != page) {
		let state = {'page_id' : page };
